...
def take_int(n, iterable):
    return int(take(n, iterable), 2)


def literal_value(it):
    is_last = not take_int(1, it)
    cur_val = take(4, it)
    if is_last:
        return cur_val
    return cur_val + literal_value(it)[0]
...
def interpret(packet):
    ...
    if type_id == 4:
        v = int(literal_value(packet), 2)
        return v
    else:
        vals = []
        len_type = take_int(1, packet)
        if len_type:
            length = take_int(11, packet)
            for _ in range(length):
                vals.append(interpret(packet))
        else:
            length = take_int(15, packet)
            subpackets = iter(take(length, packet))
            while True:
                try:
